...
because two runners are unavoidable mid-partition (and mid-handover).

  Safety   — one live node runs the agent, and the single durable row names that node.
...
event eMStore: (leader: int, owner: int, live: map[int, bool], newRuns: int, newRefills: int);
event eMRunDone;
event eMRefillDone;
...
spec OwnershipConverges observes eMRoster, eMNode, eMStore, eMRunDone, eMRefillDone, eMRowPending, eMRowDone, eMFault {
  ...
  /* The one assignment row's node_id; 0 = no row. */
  var owner: int;
  ...
  start cold state Quiet {
    ...
    on eMStore do (p: (leader: int, owner: int, live: map[int, bool], newRuns: int, newRefills: int)) { st(p); }
  ...
  fun st(p: (leader: int, owner: int, live: map[int, bool], newRuns: int, newRefills: int)) {
    owner = p.owner;
    live = p.live;
  ...
  fun converged() {
    ...
    if (liveCount == 0) {
      ...
      assert owner == 0,
        format("no node is alive, yet a durable assignment row for node {0} survives", owner);
      return;
    }
    ...
    assert owner == runner,
      format("the runner is node {0} but the durable row names node {1}", runner, owner);
  }
}
